...
// *** For fitted renormalization constant ***
...
impl<'a, F: Fn(R, R, R) -> R + Sync> ChiSquare2Params<'a, F> {
    ...
    > {
        ...
        let mut exec = Executor::new(self, solver)
            .configure(|state| state.param(init_param).max_iters(MAX_SD_ITER));
        if observe {
...
// *** For fixed renormalization constant ***

// Returns the reduced chi square computed for parameters (a, b) given the data
// [(p, d, e)] (independent variable, dependent variable, error) and the function
// ff(p, a, b). pr is the renormalization scale at which fr is multiplicatively
// renormalized as in MOM scheme with respect to its first argument
fn rencost2<F: Fn(R, R, R) -> R + Sync>(a: R, b: R, data: &[(R, R, R)], ff: &F, pr: R) -> R {
    let z = 1. / (pr * pr * ff(pr, a, b));
    let diff2s: Vec<R> = data
        .par_iter()
        .map(|(p, d, e)| {
            let val = z * ff(*p, a, b);
...
pub struct RenChiSquare2Params<'a, F: Fn(R, R, R) -> R> {
    pub ff: &'a F,
    pub data: &'a [(R, R, R)],
    pub pren: R,
}

impl<F: Fn(R, R, R) -> R + Sync> CostFunction for RenChiSquare2Params<'_, F> {
    type Param = Vec<R>;

    type Output = R;

    fn cost(&self, param: &Self::Param) -> Result<Self::Output, argmin::core::Error> {
        Ok(rencost2(param[0], param[1], self.data, self.ff, self.pren))
    }
}

impl<F: Fn(R, R, R) -> R + Sync> Gradient for RenChiSquare2Params<'_, F> {
    type Param = Vec<R>;
    ...
    fn gradient(&self, param: &Self::Param) -> Result<Self::Gradient, argmin::core::Error> {
        let (a, b) = (param[0], param[1]);
        Ok(vec![
            (rencost2(a + EPS_GRAD, b, self.data, self.ff, self.pren)
                - rencost2(a - EPS_GRAD, b, self.data, self.ff, self.pren))
                / (2. * EPS_GRAD),
            (rencost2(a, b + EPS_GRAD, self.data, self.ff, self.pren)
                - rencost2(a, b - EPS_GRAD, self.data, self.ff, self.pren))
                / (2. * EPS_GRAD),
        ])
    }
...
#[allow(clippy::type_complexity)]
impl<'a, F: Fn(R, R, R) -> R + Sync> RenChiSquare2Params<'a, F> {
    pub fn executor(
        self,
        init_param: Vec<R>,
        observe: bool,
    ) -> Executor<
        RenChiSquare2Params<'a, F>,
        SteepestDescent<MoreThuenteLineSearch<Vec<R>, Vec<R>, R>>,
        IterState<Vec<R>, Vec<R>, (), (), (), R>,
    > {
        let linesearch = MoreThuenteLineSearch::new();
