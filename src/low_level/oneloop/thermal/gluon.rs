...
pub(crate) mod inlines {
    ...
    pub fn thermal_polarization_l_landau_i<T: Num>(q: R, om: T, p: R, m: R, beta: R) -> C {
        ...
        let j_m_l_p_i = s.inv() * (om * om * j_m_t_i(q, m, beta) + p * p * j_m_l_i(q, m, beta));
        let j_0_l_p_i = s.inv() * (om * om * j_0_t_i(q, beta) + p * p * j_0_l_i(q, beta));
        let d_j_m_l_p_i =
            s.inv() * (om * om * d_j_m_t_i(q, m, beta) + p * p * d_j_m_l_i(q, m, beta));
        let d2_j_m_l_p_i =
            s.inv() * (om * om * d2_j_m_t_i(q, m, beta) + p * p * d2_j_m_l_i(q, m, beta));
